		...
		<section>
			...
			foreach($serviceman->getService_request($_SESSION['mennica_magazyn_storage_id']) as $row)
			{
				if($row['quantity'] > 0)
				{
					...
					echo "<td><input type='text' name='product_name[]' value='".$row['product_name']."'  disabled></td>";
					//echo "<td><input type='text' name='sn[]', value='".$row['sn']."' disabled='disabled'/></td>";
					echo "<td><input type='number' name='quantity[]' value='".$row['quantity']."' disabled style='width:3em'></td>";
					echo "</tr>";
				}	
			}
			echo "</table>";
