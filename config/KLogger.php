<?php
	...
	*/
	...
	class KLogger
	{
		...
		public function __construct( $log_path, $log_file, $priority )
		{
			if ( $priority == KLogger::OFF ) return;
			@mkdir($log_path);
			/*
			$path = explode("/", $log_path);
			$temp='';
			foreach($path as $k)
			{
				$temp=$temp."/".$k;
			...
			*/
			...
			$this->log_file = $log_path.$log_file;
...
//$log = new KLogger("Logs/".date("Ymd")."/", date("Ymd")."_".date("H").".txt", KLogger::DEBUG);
?>